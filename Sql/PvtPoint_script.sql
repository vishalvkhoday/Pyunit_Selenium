...
round(((((avg(Hi) + Avg(lw))/2) + sqrt(avg(Hi) * Avg(lw)))/2) +((((avg(Hi) + Avg(lw))/2) + sqrt(avg(Hi) * Avg(lw)))/2)*0.075,2) pvt_UpRange,
 day60HnL.Day60High pvt_UpRange1,
round(((((avg(Hi) + Avg(lw))/2) + sqrt(avg(Hi) * Avg(lw)))/2) -((((avg(Hi) + Avg(lw))/2) + sqrt(avg(Hi) * Avg(lw)))/2)*0.075,2) pvt_LowRange,
Day60HnL.Day60Low pvt_DownRange1,ne.[Close]Cls,ne.Volume LastVol ,avg(Avg_Vol) Avg_vol,Day60HnL.Vol60Day,s.Sector,iss.Index_Name 
from (
...
inner join (select ne1.Script_Name,max(ne1.[High]) Day60High,min(ne1.[Low]) Day60Low,avg(ne1.Volume) Vol60Day from NSE_EOD ne1
...
ne.Script_Name = Day60HnL.Script_Name
...
group by scr,ne.[Close],s.Sector,iss.Index_Name,Day60High,Day60Low,ne.Volume,Day60HnL.Vol60Day
